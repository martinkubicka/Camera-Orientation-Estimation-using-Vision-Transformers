...
DATASET_PATH = "./dataset/out_test_correct_cutouts/"
...
for fname in os.listdir(DATASET_PATH):
    ...
    cutout_path = os.path.join(DATASET_PATH, f"{base}_cutout.jpg")
    ...
    pitch_gt, yaw_gt, roll_gt, _, yaw_cutout, pitch_cutout, cutout_w, cutout_h, query_h, query_w, cutout_fov_x, cutout_fov_y = map(float, row)
    ...
    pred_eq_nd = angles_to_eq_batch(
                pred.cpu(),
                yaw_cutout, pitch_cutout,
                int(cutout_w),  int(cutout_h),
                int(query_w),   int(query_h),
                cutout_fov_x, cutout_fov_y,
            ).to(device)
...
plt.legend()
plt.tight_layout()
plt.savefig("orientation_error_curve.png", dpi=300)
