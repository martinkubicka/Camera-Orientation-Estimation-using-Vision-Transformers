...
import numpy as np
from model_one_step import OrientationRegressor
from dataset_first_look import OrientationDataset
...
def run_epoch(model,
    ...
    for batch_idx, (query_img,
                    panorama_path,
                    pitch_gt,
                    yaw_gt,
                    roll_gt) in enumerate(loader):

        query_img = query_img.to(device, non_blocking=True)
        tgt = torch.stack((pitch_gt.float(),
                           yaw_gt.float(),
                           roll_gt.float()), dim=1).to(device, non_blocking=True)
        ...
        with torch.no_grad():
            err = (pred_eq - tgt).abs()
            correct_mask = (err <= 22.5).all(dim=1)  # FOV/2
            batch_acc = correct_mask.sum().item()
...
def main():
    log_file = open("output_one_step.txt", "a") 
    ...
    model = OrientationRegressor("dinov2_vits14").to(device)
    crit_geo = GeodesicLoss()
    opt   = torch.optim.AdamW(filter(lambda p: p.requires_grad,
                                     model.parameters()), lr=lr)

    best_val_loss = float("inf")
    ckpt_path     = "model_geo_one_step.pth"
