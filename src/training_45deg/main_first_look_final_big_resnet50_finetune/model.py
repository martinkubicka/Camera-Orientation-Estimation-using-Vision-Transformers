...
def load_backbone(device):
    ...
    encoder = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V2)  # CHANGED
...
class OrientationEstimator(nn.Module):
    def __init__(self,
        ...
        self.embed_dim = 2048  # CHANGED (ResNet18 feature channels at conv head)
    ...
    def forward(self, query_img, pano):
        ...
        # CHANGED ↓ keep spatial map -> many tokens per tile
        feats_p = self.backbone(pano_tiles)                 # [B*32, 512, H', W']
        tile_patches = feats_p.flatten(2).permute(0, 2, 1)  # CHANGED: [B*32, Np, 512]
        Np = tile_patches.size(1)                           # CHANGED

        pano_seq = tile_patches.view(B, n_tiles, Np, self.embed_dim)
        pos = self.pos_embed.unsqueeze(0).unsqueeze(2)      # [1, n_tiles, 1, D]
        pano_seq = pano_seq + pos                           # CHANGED: tile-level bias kept
