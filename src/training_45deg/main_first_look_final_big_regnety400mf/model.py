...
import numpy as np
# from transformers import AutoModel  # CHANGED (not needed)

def load_backbone(device):
    # model_name = "timm/vit_pe_spatial_tiny_patch16_512.fb"  # CHANGED
    encoder = models.regnet_y_400mf(weights=models.RegNet_Y_400MF_Weights.IMAGENET1K_V1)  # CHANGED
    encoder = nn.Sequential(*list(encoder.children())[:-2])  # keep conv feature map, not global pool  # CHANGED
...
class OrientationEstimator(nn.Module):
    def __init__(self,
        ...
        self.embed_dim = 440  # CHANGED (RegNetY-400MF final feature channels)
        ...
        self.transform = T.Compose([
            T.ToTensor(),
            T.Normalize(mean=(0.485, 0.456, 0.406),  # CHANGED (ImageNet normalization)
    ...
    def forward(self, query_img, pano):
        ...
        pano_tiles = torch.stack(all_tiles, 0)  # [B*32,3,512,512]

        # CHANGED ↓ (use feature map instead of pooled feature)
        feats_p = self.backbone(pano_tiles)  # [B*32, 440, H', W']
        feats_p = feats_p.flatten(2).permute(0, 2, 1)  # [B*32, Np, 440]
        tile_patches = feats_p  # CHANGED
        ...
        pano_emb = pano_seq.reshape(B, n_tiles * Np, self.embed_dim) 

        # CHANGED ↓ query path same logic
        feats_q = self.backbone(query_img)  # [B, 440, H', W']
        query_patches = feats_q.flatten(2).permute(0, 2, 1)  # [B, Nq, 440]
