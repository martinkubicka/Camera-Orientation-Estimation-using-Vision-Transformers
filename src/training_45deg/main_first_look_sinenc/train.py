...
def ang_diff_deg_tensor(pred: torch.Tensor, tgt: torch.Tensor, period: float) -> torch.Tensor:
    return torch.remainder(pred - tgt + period/2, period) - period/2
...
def run_epoch(model, loader, crit_geo, optimizer=None, device="cpu", scaler=None):
    ...
    for batch_idx, (query_img, panorama_path, pitch_gt, yaw_gt, roll_gt) in enumerate(pbar):
        ...
        with torch.no_grad():
            err_pitch = ang_diff_deg_tensor(pred_eq[:, 0], tgt[:, 0], 180.0).abs()
            err_yaw   = ang_diff_deg_tensor(pred_eq[:, 1], tgt[:, 1], 360.0).abs()
            err_roll  = ang_diff_deg_tensor(pred_eq[:, 2], tgt[:, 2], 360.0).abs()

            err = torch.stack([err_pitch, err_yaw, err_roll], dim=1)
            
            correct_mask = (err <= 22.5).all(dim=1) # FOV / 2 -> FOV for both is 45 deg
...
def main():
    ...
    log_file.write("Training started 2nd." + "\n")
    log_file.flush()
    ...
    ckpt_path     = "model_geo_first_look_45_2nd.pth"
