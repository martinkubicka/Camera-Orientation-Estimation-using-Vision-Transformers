...
def load_dinov2(model_name: str = "dinov2_vits14", pretrained: bool = True) -> nn.Module:
...
class PoseRegressor(nn.Module):
    ...
    def forward(self, query_img, pano):
        ...
        all_tiles = []
        for img in pano:
            for i in range(0, self.num_cols * self.num_rows):
                tile = Image.open(img.replace("0_panorama.jpg", f"_{i}_panorama.jpg")).convert("RGB")
