...
class PoseRegressor(nn.Module):
    ...
    def forward(self, query_img, pano):
        pano_batch = []
        for img in pano:
            base_img = Image.open(img).convert("RGB")
            base_img = base_img.resize((4096, 2048), Image.BICUBIC)
            pano_batch.append(self.transform(base_img))

        pano_batch = torch.stack(pano_batch, 0).to(query_img.device)

        feats_p = self.backbone(pixel_values=pano_batch)
        pano_patches = feats_p.last_hidden_state[:, 1:-self.backbone.config.num_register_tokens, :] # [B, Np, D]
        pano_emb = pano_patches
