...
class PoseRegressor(nn.Module):
    def __init__(self,
        ...
        self.post_enc_pano  = nn.TransformerEncoder(enc_layer, num_layers=2)
        self.post_enc_query = nn.TransformerEncoder(enc_layer, num_layers=2)
    ...
    def forward(self, query_img, pano):
        ...
        pano_tiles = torch.stack(all_tiles, 0)  # [B*32,3,518,518] -> 8 cols * 4 rows = 32
                                                # all together for encoder processing
        # dino encoder part                              
        feats_p = self.backbone.forward_features(pano_tiles) # [B*32, Np, D] - Np number of patches 
        tile_patches = feats_p['x_norm_patchtokens']
        Np = tile_patches.size(1)
        
        # separate back (not together)
        pano_seq = tile_patches.view(B, n_tiles, Np, self.embed_dim) # [B, 32, Np, D]

        # [32, D] -> [1,32,1,D] - add position to right part (where embeddings are)
        pos = self.pos_embed.unsqueeze(0).unsqueeze(2)
        pano_seq = pano_seq + pos

        # flatten to [B, 32*Np, D] - batch, panoramas (together with patches), dim
        ...
        # ADDED 2nd version
        pano_emb = pano_emb + self.post_enc_pano(pano_emb)
        query_patches = query_patches + self.post_enc_query(query_patches)
        # END of added 2nd version
